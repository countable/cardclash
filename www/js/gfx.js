...
var get_display = function(card) {
    ...
    var el, selector = '.field', field = GAME.player.field, idx = field.indexOf(card);
    ...
    if (card.is_a) {
        ...
        if (idx == -1) {
            ...
            if (idx !== -1) idx = field.length - 1 - idx;
            selector = '.enemy';
        }

        if (idx == -1) {
            field = GAME.player.hand;
            idx = field.indexOf(card);
            selector = '.hand';
        }
        el = $$(selector + " .card")[idx];
    ...
    return {
        el: el,
        rect: rect,
        near: field && field === GAME.player.field
    };
};
